 /* This program uses King et al.'s matrix inversion technique to calculate the aerosol number density.
    ...
    The radius of aerosols affecting each wavelength are determined from Mie theory: Qext >= 3
...
int main()
{
	...
	//{
		...
		printf("day %f \n", day);
		printf("aodm1 %f", aodm[1]);
		cout << day << endl;
		//printf("hello world...");
		for(j=0;j<6;j++){
			eaodm[j]=0.02*aodm[j];
			printf("eaodm %f", eaodm[j]);
