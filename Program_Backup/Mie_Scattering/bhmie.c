...
float qext, qsca, qabs, qback, gsca;	/* efficiencies for extinction, scattering, absorption, backscattering; also cos<theta> */
...
/* ask for output file name */
  ...
  if ((fp1=fopen(outfile, "w")) == NULL)
	...
	fprintf(fp1,"Radius,Qext,Qsca,Qabs,Gsca\n");
	}

/* ask for complex index of refraction */
  printf("Input the complex index of refraction as real,imaginary (e.g., 1.5,0.1): ");
  scanf("%f,%f", &nreal,&nimag);
  cxref=Complex(nreal,nimag); /* form complex index of refraction */
/* ask for wavlength of light to use in calculation */
  i=0;
  printf("Input wavelength in micrometers: ");
  scanf("%f", &lambda);
/* do aerosol radii from 40 nm to 10000 nm */
  for(i=40; i<=10000;i++){
    r=i/1000.0;		/* convert radius to microns */
    x=2.0 * PI * r / lambda;	/* length parameter x */
    ...
    fprintf(fp1,"%f,%f,%f,%f,%f\n",r,qext,qsca,qabs,gsca);
  }

/*
for (i=1; i<=nang; i++) {
  printf(" s1 r,i %i  %f %f %f %f \n ", i, cxs1[i].r, cxs1[i].i, cxs2[i].r, cxs2[i].i);
}
*/
  fclose(fp1);
